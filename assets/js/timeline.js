...
function uagbTimelineInit() {
	...
	for ( const content of timeline ) {
		const lineInner = content.querySelector(
		...
		const timelineField = content.querySelector(
			'.uagb-timeline__field:nth-last-child(2)'
		);
		...
		if ( content.classList.contains( 'uagb-timeline__arrow-center' ) ) {
			...
			content.classList.contains( 'uagb-timeline__arrow-bottom' )
		) {
			const bottomHeight =
				timelineCardHeight - timelineEndIcon?.offsetTop;
		...
		const timelineEndIconOffsetBottom =
			height - timelineEndIcon?.getBoundingClientRect()?.top;

		const totalTimelineLineHeight =
			height -
			timelineStartIcon?.getBoundingClientRect()?.top -
			timelineEndIconOffsetBottom;
		...
		if ( startFlag < document?.documentElement?.scrollTop ) {
			const tscrollPerc =
				( ( document?.documentElement?.scrollTop - startFlag ) /
					totalTimelineLineHeight ) *
				100;
		...
		const timelineIcon = content.querySelectorAll(
		...
		if ( animateBorder.length === 0 ) {
			animateBorder = content.querySelectorAll(
		...
		for ( let j = 0; j < timelineIcon.length; j++ ) {
			timelineIconPos =
				timelineIcon[ j ].lastElementChild.getBoundingClientRect().top +
				window.scrollY;
			timelineCardPos =
				animateBorder[ j ].lastElementChild.getBoundingClientRect()
					.top + window.scrollY;

			timelineIconTop =
				timelineIconPos - document.documentElement.scrollTop;
			timelineCardTop =
				timelineCardPos - document.documentElement.scrollTop;
...
function UAGBTimelineClasses( attributes, id ) {
	...
	for ( const content of timeline ) {
		...
		let timelineChild = content.querySelectorAll(
		...
		for ( const child of timelineChild ) {
			...
			const timelineMarker = child.querySelectorAll(
				'.uagb-timeline__marker'
			)[ 0 ];
			timelineMarker.classList.remove(
				'uagb-timeline__left',
				'uagb-timeline__right'
			);

			const timeLineEventInner = child.querySelectorAll(
				'.uagb-timeline__events-inner-new'
			)[ 0 ];

			timeLineEventInner.classList.remove(
