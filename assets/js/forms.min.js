UAGBForms = {
	init( r, a, d ) {
		...
		if ( g ) {
			const o = g.querySelector( '.uagb-forms-main-form' ),
			...
			const s = o.querySelectorAll( '.uagb-forms-toggle-input' );
			if ( 0 !== s.length )
				for ( let t = 0; t < s.length; t++ )
					s[ t ].addEventListener( 'change', function () {
						var e;
						s[ t ].checked
							? ( ( e = s[ t ].getAttribute( 'data-truestate' ) ),
							  s[ t ].setAttribute( 'value', e ) )
							: ( ( e =
									s[ t ].getAttribute( 'data-falsestate' ) ),
							  s[ t ].setAttribute( 'value', e ) );
					} );
			const n = g.querySelectorAll( '.uagb-forms-checkbox-wrap' );
			if ( 0 !== n.length )
				for ( let e = 0; e < n.length; e++ ) {
					const u = n[ e ].querySelectorAll( 'input[type=checkbox]' );
					if ( 0 < u.length )
						for ( let c = 0; c < u.length; c++ )
							u[ c ].addEventListener( 'change', function () {
								var t = u[ c ].checked,
									e = u[ c ].getAttribute( 'name' );
			...
			const l = document.getElementsByClassName(
				'uagb-block-' + r.block_id
			);
			const i = l[ 0 ].children[ 0 ].children;
			for ( let e = 0; e < i.length; e++ ) {
				if (
					i[ e ].classList.contains( 'uag-col-2' ) &&
					i[ e + 1 ].classList.contains( 'uag-col-2' )
				) {
					const f = document.createElement( 'div' ),
						h =
							( ( f.className =
								'uag-col-2-wrap uag-col-wrap-' + e ),
							i[ e + 1 ].after( f ),
				...
				if (
					i[ e ].classList.contains( 'uag-col-3' ) &&
					i[ e + 1 ].classList.contains( 'uag-col-3' ) &&
					i[ e + 2 ].classList.contains( 'uag-col-3' )
				) {
					const b = document.createElement( 'div' ),
						y =
							( ( b.className =
								'uag-col-3-wrap uag-col-wrap-' + e ),
							i[ e + 2 ].after( b ),
							l[ 0 ].getElementsByClassName(
								'uag-col-wrap-' + e
							) );
				...
				if (
					i[ e ].classList.contains( 'uag-col-4' ) &&
					i[ e + 1 ].classList.contains( 'uag-col-4' ) &&
					i[ e + 2 ].classList.contains( 'uag-col-4' ) &&
					i[ e + 3 ].classList.contains( 'uag-col-4' )
				) {
					const _ = document.createElement( 'div' ),
						v =
							( ( _.className =
								'uag-col-4-wrap uag-col-wrap-' + e ),
							i[ e + 3 ].after( _ ),
							l[ 0 ].getElementsByClassName(
								'uag-col-wrap-' + e
							) );
	...
	_formSubmit( e, u, r, m, d, g ) {
		...
		if ( '' === r.afterSubmitToEmail || null === r.afterSubmitToEmail ) {
			...
			return (
				...
				l.classList.add( 'uagb-forms-success-message' ),
				! 1
			);
		}
		...
		for ( let e = 0; e < a.length; e++ ) {
			var o,
				s = document.getElementById( a[ e ].name );
			a[ e ].name.endsWith( '[]' )
				? ( ( o = a[ e ].name.replace( /[\[\]']+/g, '' ) ) in c ||
						( c[ o ] = [] ),
				  c[ o ].push( a[ e ].value ) )
				: null !== s && ( c[ s.innerHTML ] = a[ e ].value );
