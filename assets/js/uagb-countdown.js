...
UAGBCountdown = {
	...
	updateCountdown( mainSelector, data, isEditor = false, ref = null ) {
		...
		if ( isEditor ) {
			if ( data.showDays ) {
				daysWrap = ref.querySelector(
					'.wp-block-uagb-countdown__time-days'
				);
			}

			if ( data.showHours ) {
				hoursWrap = ref.querySelector(
					'.wp-block-uagb-countdown__time-hours'
				);
			}

			if ( data.showMinutes ) {
				minutesWrap = ref.querySelector(
					'.wp-block-uagb-countdown__time-minutes'
				);
			}

			secondsWrap = ref.querySelector(
				'.wp-block-uagb-countdown__time-seconds'
			);
		} else {
			if ( data.showDays ) {
				daysWrap = this.elements[ mainSelector ]?.querySelector(
					'.wp-block-uagb-countdown__time-days'
				);
			}

			if ( data.showHours ) {
				hoursWrap = this.elements[ mainSelector ]?.querySelector(
					'.wp-block-uagb-countdown__time-hours'
				);
			}

			if ( data.showMinutes ) {
				minutesWrap = this.elements[ mainSelector ]?.querySelector(
					'.wp-block-uagb-countdown__time-minutes'
				);
			}

			secondsWrap = this.elements[ mainSelector ]?.querySelector(
				'.wp-block-uagb-countdown__time-seconds'
			);
		}
