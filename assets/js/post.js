	...
	_setHeight( scope ) {
		if ( scope.length > 0 ) {
			const postWrapper = scope[ 0 ].querySelectorAll( '.slick-slide' ),
				postActive = scope[ 0 ].querySelectorAll(
			...
			Object.keys( postActive ).forEach( ( key ) => {
				const thisHeight = postActive[ key ].offsetHeight,
					blogPost = postActive[ key ].querySelector(
			...
			Object.keys( postActive ).forEach( ( key ) => {
				const selector = postActive[ key ].querySelector(
					'.uagb-post__inner-wrap'
				);
				if ( selector ) {
...
window.UAGBPostMasonry = {
	_init( $attr, $selector ) {
		...
		if (
			'none' !== $attr.paginationType &&
			'scroll' === $attr.paginationEventType
		) {
			window.addEventListener( 'scroll', function () {
				...
				const boundingClientRect =
					postItems.lastElementChild.getBoundingClientRect();
		...
		if ( 'button' === $attr.paginationEventType ) {
			if ( $scope.querySelector( '.uagb-post-pagination-button' ) ) {
				...
				$scope.querySelector( '.uagb-post-pagination-button' ).onclick =
					function () {
	...
	_callAjax( $scope, $obj, $attr, loader, append = false, count ) {
		...
		fetch( uagb_data.ajax_url, {
			...
			.then( function ( data ) {
				...
				isotope.insert(
					window.UAGBPostMasonry.createElementFromHTML( data.data )
				);
				loadStatus = true;
				...
				for ( const article of articles ) {
					...
					const image =
						article.getElementsByClassName( 'uagb-post__image' );
					if ( image[ 0 ] ) {
