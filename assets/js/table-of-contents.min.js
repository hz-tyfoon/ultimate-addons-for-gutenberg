...
UAGBTableOfContents = {
	init( e ) {
		...
		document.addEventListener( 'click', function ( e ) {
			const t = e.target,
				o = 'path' === t?.tagName || 'svg' === t?.tagName,
				l = 'uagb-toc__title' === t?.parentNode?.className;
			if ( o && l ) {
				const r = t?.closest( '.wp-block-uagb-table-of-contents' );
	...
	hyperLinks() {
		...
		if ( '' !== e && ! /[^a-z0-9_-]$/.test( e ) ) {
			...
			if ( null !== document?.querySelector( t ) ) {
				...
				null !== t &&
					scroll( { top: t - scrollOffset, behavior: 'smooth' } );
			}
		}
	},
	_showHideScroll() {
	...
	_scroll( e ) {
		...
		if ( t ) {
			...
			( scrollData = o.getAttribute( 'data-scroll' ) ),
				( scrollOffset = o.getAttribute( 'data-offset' ) );
			let e = null;
			( t = t.substring( 1 ) ),
				...
				scrollData
					? null !== e &&
					  scroll( { top: e - scrollOffset, behavior: 'smooth' } )
	...
	_run( t, u ) {
		...
		if ( o ) {
			...
			if ( 0 !== d.length ) {
				...
				function g( e = [] ) {
					...
					return (
						e.reduce(
							( e, t ) => (
								( e.some( ( e ) => e.innerText === t.innerText )
									? o
									: e
								).push( t ),
								e
				...
				for ( let e = 0; e < s.length; e++ ) {
					...
					if ( '' !== s[ e ].innerText && t.length < 1 ) {
						const a = r.getElementsByTagName( 'a' );
						var y = s[ e ].innerText;
						for ( let e = 0; e < a.length; e++ )
							if ( a[ e ].textContent === y ) {
								const i = '#toc_' + Math.random();
								a[ e ].setAttribute( 'href', i ),
									( t = i.substring( 1 ) );
							}
					}
					...
					for ( let t = 0; t < n.length; t++ ) {
						const m = '#toc_' + Math.random();
						n[ t ]
							?.querySelector( '.uag-toc__heading-anchor' )
							?.setAttribute( 'id', m.substring( 1 ) );
						const b = g(
							Array.from( r.getElementsByTagName( 'a' ) )
						);
						for ( let e = 0; e < b.length; e++ )
							b[ t ]?.setAttribute( 'href', m );
					}
				}
			}
			...
			if (
				...
				p.classList.add( 'uagb-toc__scroll-top' ),
					( p.innerHTML =
