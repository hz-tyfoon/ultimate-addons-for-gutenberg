	...
	if ( 0 !== r.length )
		for ( const a of r ) {
			...
			l < document?.documentElement?.scrollTop &&
				( l =
					( i / 100 ) *
					( ( ( document?.documentElement?.scrollTop - l ) / i ) *
						100 ) ) <
					60 + i &&
				( _.style.height = l + 'px' );
			...
			for ( let e = 0; e < d.length; e++ )
				( o =
					d[ e ].lastElementChild.getBoundingClientRect().top +
					window.scrollY ),
					( u =
						t[ e ].lastElementChild.getBoundingClientRect().top +
						window.scrollY ),
					( o = o - document.documentElement.scrollTop ),
					u - document.documentElement.scrollTop < g
						? ( t[ e ].classList.remove( 'out-view' ),
						  t[ e ].classList.add( 'in-view' ) )
...
function UAGBTimelineClasses( l, e ) {
	...
	if ( 0 !== e.length ) {
		...
		for ( const o of e ) {
			...
			let e = '',
				t =
					( n <= uagb_timeline_data.mobile_breakpoint
						? ( e = 'Mobile' )
						: n <= uagb_timeline_data.tablet_breakpoint &&
						  ( e = 'Tablet' ),
			...
			for ( const a of ( t =
				0 === t.length
					? o.querySelectorAll( '.uagb-timeline__field' )
					: t ) ) {
				a.classList.remove(
					'uagb-timeline__left',
