...
UAGBTableOfContents = {
	...
	init( id ) {
		...
		function collapseListener( event ) {
			...
			const condition1 =
				element?.tagName === 'path' || element?.tagName === 'svg'; // Check if the clicked element type is either path or SVG.
			const condition2 =
				element?.parentNode?.className === 'uagb-toc__title'; // Check if the clicked element's parent has the required class.
	...
	_run( attr, id ) {
		...
		if (
			$thisScope.querySelector( '.uag-toc__collapsible-wrap' ) !== null
		) {
			if (
				$thisScope.querySelector( '.uag-toc__collapsible-wrap' )
					.length > 0
			) {
				$thisScope
		...
		if ( undefined !== attr.mappingHeaders ) {
			attr.mappingHeaders.forEach( function ( h_tag, index ) {
				...
				h_tag === true
					? allowedHTags.push( 'h' + ( index + 1 ) )
					: null;
			} );
			allowedHTagStr =
				null !== allowedHTags ? allowedHTags.join( ',' ) : '';
		}
		...
		if ( 0 !== allHeader.length ) {
			const tocListWrap = $thisScope.querySelector(
				'.uagb-toc__list-wrap'
			);
			...
			for ( let i = 0; i < divsArr.length; i++ ) {
				...
				for ( let k = 0; k < duplicateHeadings.length; k++ ) {
					...
					const aTags = Array.from(
						tocListWrap.getElementsByTagName( 'a' )
					);
					const duplicateHeadingsInTOC =
						ArrayOfDuplicateElements( aTags );
					for ( let l = 0; l < duplicateHeadingsInTOC.length; l++ ) {
						duplicateHeadingsInTOC[ k ]?.setAttribute(
							'href',
							randomID
						);
					}
