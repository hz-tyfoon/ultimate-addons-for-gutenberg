...
UAGBCounter = {
	...
	init( mainSelector, data = {} ) {
		...
		if (
			typeof this.elements.counterWrapper !== 'undefined' &&
	...
	getElement( selector, childSelector = null ) {
		...
		if ( domElement ) {
			...
			if ( editorCanvas && editorCanvas.contentDocument ) {
				domElement =
					editorCanvas.contentDocument.querySelector( selector );
				if ( childSelector ) {
					return ( domElement =
						domElement.querySelector( childSelector ) );
				}
			}
		}
		return domElement;
	...
	_numberCount( data ) {
		const that = this;
		const el = this.elements.counterWrapper.querySelector(
			'.uagb-counter-block-number'
		);
		if ( typeof el !== 'undefined' && el ) {
			const countUp = new window.countUp.CountUp(
				el,
				that._getEndNumber( data ),
				{
	...
	_triggerBar( el, data ) {
		...
		const startWidth =
			that._getStartNumber( data ) < that._getTotalNumber( data )
				? Math.ceil(
						( that._getStartNumber( data ) /
							that._getTotalNumber( data ) ) *
							100
				  )
				: 100;
		const endWidth =
	...
	_getStartNumber( data ) {
		...
		return data.startNumber ||
			parseFloat( data.startNumber ) === parseFloat( 0 )
