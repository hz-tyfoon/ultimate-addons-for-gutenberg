document.addEventListener( 'UAGModalEditor', function ( e ) {
	UAGBModal.init(
		'.uagb-block-' + e.detail.block_id,
		e.detail.device_type,
		true
	);
} );
...
window.UAGBModal = {
	init( mainSelector, deviceType, isAdmin ) {
		...
		if ( typeof modalWrapper !== 'undefined' && modalWrapper ) {
			const modalTrigger = modalWrapper.querySelector(
				'.uagb-modal-trigger'
			);
			...
			if ( typeof modalTrigger !== 'undefined' && modalTrigger ) {
				...
				const innerModal =
					modalWrapper.querySelector( '.uagb-modal-popup' );
				...
				const closeModal = document_element.querySelector(
					`${ mainSelector } .uagb-modal-popup-close`
				);

				closeModal.addEventListener( 'click', function () {
					const modalPopup = document_element.querySelector(
						`${ mainSelector }.uagb-modal-popup`
					);
					if (
						typeof modalPopup !== 'undefined' &&
