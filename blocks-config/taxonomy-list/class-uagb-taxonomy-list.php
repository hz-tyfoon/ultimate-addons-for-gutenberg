...
if ( ! class_exists( 'UAGB_Taxonomy_List' ) ) {
	...
	class UAGB_Taxonomy_List {
		 ...
		 */
		public function get_terms_hierarchical( $taxonomy_type, $parent_id = 0, $show_empty_taxonomy ) {
			$args = array(
		...
		public function display_terms_recursive( $term, $taxonomy_type, $showCount, $seperatorStyle, $title_tag, $show_hierarchy ) {
			if ( $term instanceof WP_Term ) {
				...
				echo wp_kses_post(
					...
					'<' . esc_html( $title_tag ) . ' class="uagb-tax-link-wrap"><a class="uagb-tax-link" href="' . esc_url( $child_link ) . '">' . esc_html( $term->name ) . '</a>' . 
					( $showCount ? ' (' . esc_attr( $term->count ) . ') ' : '' ) . 
					'<' . esc_html( $title_tag ) . '>' . 
					( 'none' !== $seperatorStyle ? '<div class="uagb-tax-separator"></div>' : '' ) . 
					'</li>'
				);

				if ( $show_hierarchy && ! empty( $term->children ) && is_array( $term->children ) ) {
					foreach ( $term->children as $child ) {
						echo '<ul class="uagb-taxonomy-list-children">';
