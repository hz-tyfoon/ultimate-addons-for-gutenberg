...
if ( ! class_exists( 'UAGB_Loader' ) ) {
	...
	final class UAGB_Loader {
		...
		public function __construct() {
			...
			add_action( 'init', array( $this, 'update_google_maps_api_url_on_a_post' ) );
		...
		public function update_google_maps_api_url_on_a_post( $post = false ) {
			...
			$post->post_content = serialize_blocks( $this->update_google_maps_api_url_on_a_blocks( $blocks ) );

			wp_update_post( $post );
		}
		/**
		 * Update Google Maps API URl in Bulk for all Blocks.
		 * 
