...
const SelectedFontFamilies = () => {
	...
	const enableSelectedFontFamilies = useSelector(
		( state ) => state.enableSelectedFontFamilies
	);
	const selectedFontFamilies = useSelector(
		( state ) => state.selectedFontFamilies
	);
	const enableSelectedFontFamiliesStatus =
		'disabled' === enableSelectedFontFamilies ? false : true;
	...
	const updateEnableSelectedFontFamilies = () => {
		...
		dispatch( {
			type: 'UPDATE_ENABLE_SELECTED_FONT_FAMILIES',
			payload: assetStatus,
		} );
	...
	const updateSelectedFontFamilies = ( font ) => {
		if ( enableSelectedFontFamilies === 'enabled' ) {
			dispatch( {
				type: 'UPDATE_SELECTED_FONT_FAMILIES',
				payload: font,
			} );
	...
	return (
		<section className="block border-b border-solid border-slate-200 px-12 py-8 justify-between">
			<div className="mr-16 w-full flex items-center">
				...
				<Switch
					...
					className={ classNames(
						enableSelectedFontFamiliesStatus
							? 'bg-spectra'
							: 'bg-slate-200',
			...
			<p className="mt-2 w-9/12 text-sm text-slate-500">
				{ __(
					'Spectra offers 700+ Google font options. If this is overwhelming for your clients, you can use this option to show only limited number of fonts in the block settings.',
					'ultimate-addons-for-gutenberg'
				) }
			</p>
			<Select
