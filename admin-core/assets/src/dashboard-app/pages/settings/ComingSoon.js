...
const ComingSoon = () => {
	...
	const enableComingSoonMode = useSelector(
		( state ) => state.enableComingSoonMode
	);
	const comingSoonPage = useSelector( ( state ) => state.comingSoonPage );
	const enableComingSoonModeStatus =
		'disabled' === enableComingSoonMode ? false : true;
	...
	return (
		<section className="block border-b border-solid border-slate-200 px-12 py-8 justify-between">
			...
			<p className="mt-2 w-9/12 text-sm text-slate-500">
				{ __(
					'Is your website still in the making? Is it not yet ready for other people to see?',
					'ultimate-addons-for-gutenberg'
				) }
			</p>
			<p className="mt-2 w-9/12 text-sm text-slate-500">
				{ __(
					'Enable the coming soon mode. Only the page you select below will be displayed to people who are not logged in.',
					'ultimate-addons-for-gutenberg'
				) }
			</p>
			<Select
