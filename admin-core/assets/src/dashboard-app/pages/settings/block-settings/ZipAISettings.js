...
const ZipAISettings = () => {
    ...
    const zipAiAssistantStatus = currentZipAiStatus?.ai_assistant;

	// Update the Zip AI status.
    const updateZipAiStatus = () => {

		// Toggle the status.
        const status = ! zipAiAssistantStatus;
        ...
        const data = {
            security: uag_react.zip_ai_assistant_status_nonce,
        ...
        const getApiDataFetch = getApiData( {
            ...
            action: 'uag_zip_ai_assistant_status',
            data,
        } );
		...
		<>
			<section className="block border-b border-solid border-slate-200 px-12 py-8 justify-between">  
				<div className="mr-16 w-full flex items-center">
					...
					<Switch
						checked={ zipAiAssistantStatus }
						onChange={ updateZipAiStatus }
						className={ uagbClassNames( [
							zipAiAssistantStatus ? 'bg-spectra' : 'bg-slate-200',
					...
					>
						<span
							aria-hidden="true"
							className={ uagbClassNames( [
								zipAiAssistantStatus ? 'translate-x-5' : 'translate-x-0',
				...
				<div className="mr-16 mt-2 w-full flex items-start">
					<div className="w-9/12">
						<p className="text-sm text-slate-500">
							{ __( 'Enables the Zip AI Assistant sidebar.', 'ultimate-addons-for-gutenberg' ) }
						</p>
					</div>
				</div>
			</section>
