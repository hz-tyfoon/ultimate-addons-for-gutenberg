...
const Settings = () => {
	...
	const navigation = [
		...
		{
			name: __( 'Templates', 'ultimate-addons-for-gutenberg' ),
			slug: 'templates',
		...
		{
			name: __( 'Block Settings', 'ultimate-addons-for-gutenberg' ),
			slug: 'block-settings',
			icon: SettingsIcons[ 'block-settings' ],
		},
		{
			name: __( 'Coming Soon', 'ultimate-addons-for-gutenberg' ),
			slug: 'coming-soon',
			icon: SettingsIcons[ 'coming-soon' ],
		},
	];

	if ( uag_react.spectra_pro_status ) {
		navigation.push( {
			name: __( 'License', 'ultimate-addons-for-gutenberg' ),
			slug: 'license',
	...
	useEffect( () => {
		...
		let activeSettingsTabFromHash =
			activeHash && 'settings' === activePath
				? activeHash
				: 'global-settings';
		if ( uag_react.spectra_pro_status && ! uag_react.license_status ) {
			activeSettingsTabFromHash =
				activeHash && 'settings' === activePath
					? activeHash
					: 'license';
			history.push( {
					...
					<div className="lg:col-span-9 border-l spectra__settings">
						{ 'global-settings' === activeSettingsNavigationTab && (
							<>
								...
								<AutoBlockRecovery />
								{ 'yes' ===
									uag_react.global_data
										.uagb_old_user_less_than_2 && (
									<LoadFontAwesome5 />
								) }
							</>
						) }
