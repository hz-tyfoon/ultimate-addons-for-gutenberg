...
export default function MyAccount() {
	const [ regenerateAssetsState, setRegenerateAssetsState ] =
		useState( false );
	const licenseStatus = uag_react.license_status;
	const licenseTriggerName = licenseStatus
		? 'bsf_deactivate_license'
		: 'bsf_activate_license';
	...
	const licenseMessage = licenseStatus
		...
		: __(
				`Please enter your valid license key below to activate Spectra Pro!`,
				'ultimate-addons-for-gutenberg'
		  );
							...
							<div className="mt-4 flex w-full">
								<div className="mr-5 h-10 relative flex-1">
									<input
										...
										value={ uag_react.bsf_graupi_nonce }
									/>
									<input
										type="hidden"
									...
									<input
										className={ `w-full h-10 text-sm placeholder-slate-400 transition spectra-admin__input-field ${
											licenseStatus
												? 'spectra-admin__input-field--read-only'
												: ''
										}` }
										id={
											licenseStatus
												? 'spectra_license_key'
												: 'bsf_license_manager[license_key]'
										}
										name={
											licenseStatus
												? 'spectra_license_key'
