...
const BlockCard = ( props ) => {
	const { admin_categories, link, slug, title, deprecated, pro_filler } =
		props.blockInfo;
	...
	const blockActivationStatus =
		'disabled' === blocksStatuses[ slug ] && ! isCoreBlock ? false : true;
	...
	return pro_filler ? null : (
		...
		>
			<div className="flex-shrink-0">{ UAGB_Block_Icons[ slug ] }</div>
			...
			{ pro_filler ? (
				...
				<div
					...
					title={
						isCoreBlock
							? __(
									'This is a Core Spectra block & cannot be disabled.',
