...
const FilterTabs = () => {
	...
	const activeBlocksFilterTab = useSelector(
		( state ) => state.activeBlocksFilterTab
	);
	...
	useEffect( () => {
		...
		dispatch( {
			type: 'UPDATE_BLOCKS_ACTIVE_FILTER_TAB',
	...
	const activateAllBlocks = () => {
		...
		for ( const block in blocksStatuses ) {
			if (
				...
				( ! categoriesBlocks[ activeBlocksFilterTab ] ||
					! categoriesBlocks[ activeBlocksFilterTab ].includes(
						block
					) )
			) {
				continue;
		...
		if (
			'extensions' === activeBlocksFilterTab ||
			'all' === activeBlocksFilterTab
		) {
			// Update Extensions Statuses.
			dispatch( {
				type: 'UPDATE_ENABLE_MASONRY_EXTENSION',
				payload: 'enabled',
			} );
			dispatch( {
				type: 'UPDATE_ENABLE_DISPLAY_CONDITIONS',
				payload: 'enabled',
			} );
			dispatch( {
				type: 'UPDATE_ENABLE_RESPONSIVE_CONDITIONS',
				payload: 'enabled',
			} );
			dispatch( {
				type: 'UPDATE_ENABLE_DYNAMIC_CONTENT_EXTENSION',
				payload: 'enabled',
			} );
		}
	...
	const renderTabClassNames = ( tabName ) => {
		...
		if ( tabName === activeBlocksFilterTab ) {
			tabClasses +=
				'pro' === tabName
					? 'text-spectra active:text-spectra focus:text-spectra hover:text-spectra'
					: 'text-slate-800 active:text-slate-800 focus:text-slate-800 hover:text-slate-800';
			tabClasses += ' bg-white border-transparent shadow shadow-focused';
		} else {
			tabClasses +=
				'pro' === tabName
					? 'text-spectra border-indigo-100 bg-indigo-50 focus:text-spectra active:text-spectra hover:text-spectra'
					: 'text-slate-500 border-slate-200 focus:text-slate-500 active:text-slate-500 hover:text-slate-500';
