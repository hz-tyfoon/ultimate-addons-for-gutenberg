...
const Render = ( props ) => {
	...
	const settings = {
		...
		slidesToShow:
			deviceType === 'Desktop'
				...
				: getFallbackNumber( mcolumns, 'columns', blockName ), // eslint-disable-line no-nested-ternary
		slidesToScroll: 1,
		autoplaySpeed: getFallbackNumber(
			autoplaySpeed,
			'autoplaySpeed',
			blockName
		),
	...
	// image size.
	const imageArray =
		attributes.test_block[ 0 ]?.image ||
		attributes.test_block[ 1 ]?.image ||
		attributes.test_block[ 2 ]?.image;
	let url = '';
	let urlCheck = '';
