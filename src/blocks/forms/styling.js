...
import getAttributeFallback, {
	getFallbackNumber,
} from '@Controls/getAttributeFallback';
...
function styling( props ) {
	...
	const fieldGapFallback = getFallbackNumber(
		fieldGap,
		'fieldGap',
		blockName
	);
	const labelGapFallback = getFallbackNumber(
		labelGap,
		'labelGap',
		blockName
	);
	...
	const toggleSizeNumber = getFallbackNumber(
		toggleSize,
		'toggleSize',
		blockName
	);
	const toggleSizeNumberTablet = isNaN( toggleSizeTablet )
		? toggleSizeNumber
		: toggleSizeTablet;
	const toggleSizeNumberMobile = isNaN( toggleSizeMobile )
		? toggleSizeNumberTablet
		: toggleSizeMobile;
	const toggleSizeTabletFallback = getFallbackNumber(
		toggleSizeTablet,
		'toggleSizeTablet',
		blockName
	);
	const toggleSizeMobileFallback = getFallbackNumber(
		toggleSizeMobile,
		'toggleSizeMobile',
		blockName
	);
	...
	// Used for Calculations, needs to be numeric.
	const toggleWidthSizeNumber = getFallbackNumber(
		toggleWidthSize,
		'toggleWidthSize',
		blockName
	);
	const toggleWidthSizeNumberTablet = isNaN( toggleWidthSizeTablet )
		? toggleWidthSizeNumber
		: toggleWidthSizeTablet;
	...
	const inputBorderTablet = generateBorderCSS(
		props.attributes,
		'field',
		'tablet'
	);
	const inputBorderMobile = generateBorderCSS(
		props.attributes,
		'field',
		'mobile'
	);

	const toggleBorder = generateBorderCSS(
		props.attributes,
		'checkBoxToggle'
	);
	const toggleBorderTablet = generateBorderCSS(
		props.attributes,
		'checkBoxToggle',
		'tablet'
	);
	const toggleBorderMobile = generateBorderCSS(
		props.attributes,
		'checkBoxToggle',
		'mobile'
	);
	...
	const toggleBorderRadiusTLTabletFallback =
		...
		'px' !== toggleBorderTablet[ 'border-top-left-radius' ]
			? toggleBorderTablet[ 'border-top-left-radius' ]
			: toggleBorderRadiusTLFallback;
	...
	const toggleBorderRadiusTLMobileFallback =
		...
		'px' !== toggleBorderMobile[ 'border-top-left-radius' ]
			? toggleBorderMobile[ 'border-top-left-radius' ]
	...
	let toggleBorderLFallback =
		undefined !== toggleBorder[ 'border-left-width' ]
			? getAttributeFallback(
					toggleBorder[ 'border-left-width' ],
	...
	selectors = {
		...
		' .uagb-forms-main-form .uagb-forms-phone-flex': {
			'height': `calc(${
				inputLineHeight
					? generateCSSUnit( inputLineHeight, inputLineHeightType )
					: '2em'
			} + ${ generateCSSUnit(
				paddingFieldTop,
				paddingFieldUnit
		...
		' .uagb-slider:before': {
			...
			'top': generateCSSUnit(
				parseInt( ( 20 + toggleWidthSizeNumber ) / 6 ),
				'px'
			),
		...
		' .uagb-forms-radio-wrap input[type=radio]:checked + label:before': {
			...
			'font-size':
				'calc(' + toggleSizeNumber + toggleSizeType + ' / 1.2 )',
		},
	...
	tabletSelectors = {
		...
		' .uagb-forms-radio-wrap input[type=radio]:checked + label:before': {
			'font-size':
				'calc(' + toggleSizeNumberTablet + toggleSizeType + ' / 1.2 )',
		},
		' .uagb-forms-radio-wrap input[type=radio] + label:before': {
		...
		' .uagb-switch .uagb-slider:before': {
			...
			'top': generateCSSUnit(
				parseInt( ( 20 + toggleWidthSizeNumberTablet ) / 6 ),
				'px'
			),
			'bottom': generateCSSUnit(
				parseInt( ( 20 + toggleWidthSizeNumberTablet ) / 6 ),
		...
		' .uagb-slider.round': {
			'border-radius': generateCSSUnit(
				20 + toggleWidthSizeNumberTablet,
				'px'
			),
		},
		' .uagb-switch input:checked + .uagb-slider:before ': {
	...
	mobileSelectors = {
		...
		' .uagb-forms-radio-wrap input[type=radio]:checked + label:before': {
			'font-size':
				'calc(' + toggleSizeNumberMobile + toggleSizeType + ' / 1.2 )',
		},
		' .uagb-forms-radio-wrap input[type=radio] + label:before': {
		...
		' .uagb-switch': {
			'height': `calc(${ toggleBorderTMobileFallback } + ${ toggleBorderBMobileFallback } + ${ generateCSSUnit(
				parseInt(
					20 +
						toggleWidthSizeNumberMobile +
						( 20 + toggleWidthSizeNumberMobile ) / 3
				),
				'px'
			) })`,
			'width': `calc(${ toggleBorderLMobileFallback } + ${ toggleBorderRMobileFallback } + ${ generateCSSUnit(
				parseInt(
					( 20 + toggleWidthSizeNumberMobile ) * 2.5 +
						( 20 + toggleWidthSizeNumberMobile ) / 3
				),
				'px'
			) })`,
		...
		' .uagb-switch .uagb-slider:before': {
			...
			'top': generateCSSUnit(
				parseInt( ( 20 + toggleWidthSizeNumberMobile ) / 6 ),
				'px'
			),
			'bottom': generateCSSUnit(
				parseInt( ( 20 + toggleWidthSizeNumberMobile ) / 6 ),
		...
		' .uagb-slider.round': {
			'border-radius': generateCSSUnit(
				20 + toggleWidthSizeNumberMobile,
				'px'
			),
		},
		' .uagb-switch input:checked + .uagb-slider:before ': {
