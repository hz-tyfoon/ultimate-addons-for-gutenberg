...
const UAGCopyPasteStyles = () => {
	...
	useEffect( () => {
		const spectraCopyPasteStyles = getUAGEditorStateLocalStorage(
			'spectraCopyPasteStyles'
		);

		if ( ! spectraCopyPasteStyles ) {
			spectraCopyPasteStylesSetter.setItem(
				'spectraCopyPasteStyles',
				JSON.stringify( {} )
			);
		}
		...
		if ( spectraCopyPasteStyles ) {
			for ( const block in spectraCopyPasteStyles ) {
				const hoursSinceStylesSaved =
					Math.abs(
						Date.now() -
							spectraCopyPasteStyles[ block ].stylesSavedTimeStamp
					) / 36e5;
			...
			spectraCopyPasteStylesSetter.setItem(
				'spectraCopyPasteStyles',
				JSON.stringify( spectraCopyPasteStyles )
			);
		}
	...
	const copyStylesHandler = () => {
		...
		const { getSelectedBlock, hasMultiSelection, getMultiSelectedBlocks } =
			...
			if ( blockAttributes && spectraCopyPasteStyles ) {
				Object.keys( blockAttributes ).map( ( attribute ) => {
					if ( blockAttributes[ attribute ].UAGCopyPaste ) {
						const key =
							blockAttributes[ attribute ].UAGCopyPaste.styleType;
	...
	const pasteBlockStyles = ( blockData ) => {
			...
			if ( blockAttributes && pasteStyle ) {
				...
				if ( innerBlocks ) {
					...
					innerBlocks.map( ( childBlock, index ) => {
						const childName = childBlock.name.replace(
							'uagb/',
							''
						);
						const blockAttributes =
							allBlocksAttributes[ childName ];

						if (
							pasteStyle.innerblocks[ index ].name ===
							'uagb/' + childName
						) {
		...
		if ( name.includes( 'core/' ) ) {
			...
			pasteStyle =
				spectraCopyPasteStyles[ `core-${ selectedBlockName }-styles` ];
		...
		<BlockControls group="block">
			...
			{ showPopup && (
				...
				>
					...
					>
						{ SettingsIcons.paste }
						{ __( 'Paste ', 'ultimate-addons-for-gutenberg' ) +
							stylesText }
					</MenuItem>
				</Popover>
			) }
...
const displayUAGCopyPasteSettingConditionally = createHigherOrderComponent(
	( BlockEdit ) => {
		return ( props ) => {
			...
			if ( selectedBlock ) {
				...
				if (
					( singleSelectedBlockName.includes( 'uagb/' ) ||
