...
function styling( props ) {
	...
	const iconSizeTabFallback = getFallbackNumber(
		iconSizeTab,
		'iconSizeTab',
		blockName
	);
	const iconSizeMobFallback = getFallbackNumber(
		iconSizeMob,
		'iconSizeMob',
		blockName
	);
	const highlightWidthFallback = getFallbackNumber(
		highlightWidth,
		'highlightWidth',
		blockName
	);
	const highlightWidthTabletFallback = getFallbackNumber(
		highlightWidthTablet,
		'highlightWidthTablet',
		blockName
	);
	const highlightWidthMobileFallback = getFallbackNumber(
		highlightWidthMobile,
		'highlightWidthMobile',
		blockName
	);
	...
	const posTopTab = ! isNaN( titleTopPaddingTablet )
		? titleTopPaddingTablet
		: titleTopPadding;
	const posLeftTab = ! isNaN( titleLeftPaddingTablet )
		? titleLeftPaddingTablet
		: titleLeftPadding;
	const posRightTab = ! isNaN( titleRightPaddingTablet )
		? titleRightPaddingTablet
		: titleRightPadding;
	const posClassicTab = ! isNaN( highlightWidthTablet )
		? highlightWidthTablet
		: highlightWidthFallback;
	const posTopUnitTab = ! isNaN( titleTopPaddingTablet )
		? tabletTitlePaddingUnit
		: titlePaddingUnit;
	const posLeftUnitTab = ! isNaN( titleLeftPaddingTablet )
		? tabletTitlePaddingUnit
		: titlePaddingUnit;
	const posRightUnitTab = ! isNaN( titleRightPaddingTablet )
		? tabletTitlePaddingUnit
		: titlePaddingUnit;

	const posTopMob = ! isNaN( titleTopPaddingMobile )
		? titleTopPaddingMobile
		: posTopTab;
	const posLeftMob = ! isNaN( titleLeftPaddingMobile )
		? titleLeftPaddingMobile
		: posLeftTab;
	const posRightMob = ! isNaN( titleRightPaddingMobile )
		? titleRightPaddingMobile
		: posRightTab;
	const posClassicMob = ! isNaN( highlightWidthMobile )
		? highlightWidthMobile
		: posClassicTab;
	const posTopUnitMob = ! isNaN( titleTopPaddingMobile )
		? mobileTitlePaddingUnit
		: posTopUnitTab;
	const posLeftUnitMob = ! isNaN( titleLeftPaddingMobile )
		? mobileTitlePaddingUnit
		: posLeftUnitTab;
	const posRightUnitMob = ! isNaN( titleRightPaddingMobile )
		? mobileTitlePaddingUnit
		: posRightUnitTab;
	...
	if ( 'modern' === layout ) {
		...
		selectors[
			...
			generateCSSUnit( highlightWidthFallback, 'px' ) +
			' solid ' +
			noticeColor;
		selectors[
			' .rich-text.block-editor-rich-text__editable.uagb-notice-text'
		...
		mobileSelectors[
			...
			generateCSSUnit( highlightWidthMobileFallback, 'px' ) +
			' solid ' +
			noticeColor;
		mobileSelectors[
			' .rich-text.block-editor-rich-text__editable.uagb-notice-title'
		...
		tabletSelectors[
			...
			generateCSSUnit( highlightWidthTabletFallback, 'px' ) +
			' solid ' +
			noticeColor;
		tabletSelectors[
			' .rich-text.block-editor-rich-text__editable.uagb-notice-title'
		...
		tabletSelectors[ '.uagb-inline_notice__align-right svg' ] = {
			'left': `calc(${ generateCSSUnit(
				posLeftTab,
				posLeftUnitTab
			) } + ${ generateCSSUnit( posClassicTab, 'px' ) })`,
		};
		mobileSelectors[ '.uagb-inline_notice__align-right svg' ] = {
			'left': `calc(${ generateCSSUnit(
				posLeftMob,
				posLeftUnitMob
			) } + ${ generateCSSUnit( posClassicMob, 'px' ) })`,
		};
	}
