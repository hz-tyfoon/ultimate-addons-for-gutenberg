...
const UAGBContainer = ( props ) => {
	...
	useEffect( () => {
		...
		const excludeBlockNames = [ 'uagb/container', 'uagb/loop-builder' ];
		const excludeBlockExists = parentBlockName.some( item => excludeBlockNames.includes( item.name ) );
		
		if ( ! excludeBlockExists ) {
