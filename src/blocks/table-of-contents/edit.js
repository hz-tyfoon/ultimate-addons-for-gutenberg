...
const UAGBTableOfContentsEdit = ( props ) => {
	...
	useSelect( ( select ) => {
		...
		if ( iframeEl ) {
			locateRootContainerInsideIframe =
				iframeEl.contentDocument.getElementsByClassName(
					'is-root-container'
				);
			headerArray =
				locateRootContainerInsideIframe[ 0 ]?.querySelectorAll(
					'h1, h2, h3, h4, h5, h6'
				);
		} else {
		...
		const excludeBlock = document.querySelectorAll(
			'.uagb-toc-hide-heading'
		);
		if ( excludeBlock ) {
			excludeBlock.forEach( function ( heading ) {
				const innerHeading = heading.querySelectorAll(
					'h1, h2, h3, h4, h5, h6'
				);
				if ( innerHeading ) {
